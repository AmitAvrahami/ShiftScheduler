...
describe('weekUtils (Frontend)', () => {
    describe('getCurrentWeekId', () => {
        ...
        it('returns the correct weekId for a known date', () => {
            vi.useFakeTimers();
            vi.setSystemTime(new Date('2024-03-01T12:00:00Z')); // Friday, week 09
            expect(getCurrentWeekId()).toBe('2024-W09');

            vi.setSystemTime(new Date('2024-01-01T12:00:00Z')); // Monday, week 01
    ...
    describe('getWeekDates', () => {
        ...
        it('dates are correct for a known weekId (2024-W10)', () => {
            ...
            expect(dates[0].getMonth()).toBe(2); // March is 2
    ...
    describe('getDeadline', () => {
        ...
        it('the Monday belongs to the correct week', () => {
            ...
            expect(deadline.toISOString().startsWith('2024-03-04')).toBe(true);
        });
    });
    ...
    describe('isDeadlinePassed', () => {
        ...
        it('returns false if today is Monday before 23:59 (edge case)', () => {
            vi.useFakeTimers();
            vi.setSystemTime(new Date(2024, 2, 4, 23, 58, 0));
            expect(isDeadlinePassed('2024-W10')).toBe(false);

            vi.setSystemTime(new Date(2024, 2, 5, 0, 0, 0)); // Tuesday
            expect(isDeadlinePassed('2024-W10')).toBe(true);
        });
    });
});
